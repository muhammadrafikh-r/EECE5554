data = readmatrix('/home/rayyan/catkin_ws/EECE5554/lab_3/src/Analysis/imu.csv');
...
x_theta=cumsum(GyroX, 1)*t0;
y_theta=cumsum(GyroY, 1)*t0;
z_theta=cumsum(GyroZ, 1)*t0;
...
for i = 1:numel(mx)
    mi = mx(i);
    avarx(i,:) = sum( ...
        (x_theta(1+2*mi:Lx) - 2*x_theta(1+mi:Lx-mi) + x_theta(1:Lx-2*mi)).^2, 1);
end
...
for i = 1:numel(my)
    mi = my(i);
    avary(i,:) = sum( ...
        (y_theta(1+2*mi:Ly) - 2*y_theta(1+mi:Ly-mi) + y_theta(1:Ly-2*mi)).^2, 1);
end
...
for i = 1:numel(mz)
    mi = mz(i);
    avarz(i,:) = sum( ...
        (z_theta(1+2*mi:Lz) - 2*z_theta(1+mi:Lz-mi) + z_theta(1:Lz-2*mi)).^2, 1);
end
...
title('Allan Deviation in Gyro X with Noise Parameters')
xlabel('\tau')
ylabel('\sigma(\tau)')
...
title('Allan Deviation in Gyro Y with Noise Parameters')
xlabel('\tau')
ylabel('\sigma(\tau)')
...
title('Allan Deviation in Gyro Z with Noise Parameters')
xlabel('\tau')
ylabel('\sigma(\tau)')
legend('$\sigma (rad/s)$', '$\sigma_N ((rad/s)/\sqrt{Hz})$', ...
    '$\sigma_K ((rad/s)\sqrt{Hz})$', '$\sigma_B (rad/s)$', 'Interpreter', 'latex')
